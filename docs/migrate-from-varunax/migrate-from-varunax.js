 ...
 * 
 * Migrates the VarunaX codebase to VarunaX by:
 * 1. Replacing all references to "varunax" with "varunax" (case-insensitive)
 ...
 * 
 * Usage:
 *   node scripts/migrate-from-varunax.js [--dry-run] [--skip-git]
 * 
 * Options:
...
const REPLACEMENTS = [
  // Exact case matches
  { from: 'varunax', to: 'varunax', flags: 'g' },
  { from: 'VarunaX', to: 'VarunaX', flags: 'g' },
  { from: 'VARUNAX', to: 'VARUNAX', flags: 'g' },
  
  // Package scope (if using @varunax/)
  { from: '@varunax/', to: '@varunax/', flags: 'g' },
  
  // Environment variables
  { from: 'VARUNAX_', to: 'VARUNAX_', flags: 'g' },
];
...
function updatePackageJson() {
  ...
  try {
    ...
    // Update name
    if (pkg.name && pkg.name.toLowerCase().includes('varunax')) {
      pkg.name = pkg.name.replace(/varunax/gi, match => {
        if (match === 'varunax') return 'varunax';
        if (match === 'VarunaX') return 'VarunaX';
        if (match === 'VARUNAX') return 'VARUNAX';
        return match;
      });
...
function updateGit() {
  ...
  try {
    ...
    if (currentBranch.toLowerCase().includes('varunax')) {
      const newBranch = currentBranch.replace(/varunax/gi, match => {
        if (match === 'varunax') return 'varunax';
        if (match === 'VarunaX') return 'VarunaX';
        if (match === 'VARUNAX') return 'VARUNAX';
        return match;
...
function generateReport() {
  ...
  if (isDryRun) {
    ...
    log('  4. Commit changes: git add . && git commit -m "Migrate from VarunaX to VarunaX"', colors.reset);
    log('', colors.reset);
    log('If you need to rollback, run:', colors.yellow);
    log('  node scripts/migrate-from-varunax.js --rollback', colors.yellow);
  }
}
...
╚╗╔╝├─┤├┬┘│ ││││├─┤╠╩╗  ║║║││ ┬├┬┘├─┤ │ ││ ││││
 ╚╝ ┴ ┴┴└─└─┘┘└┘┴ ┴╩ ╩  ╩ ╩┴└─┘┴└─┴ ┴ ┴ ┴└─┘┘└┘
  ...
  log('VarunaX → VarunaX Migration Script', colors.bright);
