> [!NOTE]
> **[VarunaX Lite achieves a 96.15% success rate on a hint-free, source-aware XBOW benchmark. &rarr;](https://github.com/KeygraphHQ/varunax/tree/main/xben-benchmark-results/README.md)**
...
<img src="./assets/varunax-screen.png" alt="VarunaX Screen" width="100%">

# VarunaX is your fully autonomous AI pentester.

VarunaX’s job is simple: break your web app before anyone else does. <br />
The Red Team to your vibe-coding Blue team. <br />
Every Claude (coder) deserves their VarunaX.
...
VarunaX is an AI pentester that delivers actual exploits, not just alerts.

VarunaX's goal is to break your web app before someone else does. It autonomously hunts for attack vectors in your code, then uses its built-in browser to execute real exploits, such as injection attacks, and auth bypass, to prove the vulnerability is actually exploitable.

**What Problem Does VarunaX Solve?**

Thanks to tools like Claude Code and Cursor, your team ships code non-stop. But your penetration test? That happens once a year. This creates a *massive* security gap. For the other 364 days, you could be unknowingly shipping vulnerabilities to production.

VarunaX closes this gap by acting as your on-demand whitebox pentester. It doesn't just find potential issues. It executes real exploits, providing concrete proof of vulnerabilities. This lets you ship with confidence, knowing every build can be secured.

> [!NOTE]
> **From Autonomous Pentesting to Automated Compliance**
>
> VarunaX is a core component of the **Keygraph Security and Compliance Platform**.
>
> While VarunaX automates the critical task of penetration testing for your application, our broader platform automates your entire compliance journey—from evidence collection to audit readiness. We're building the "Rippling for Cybersecurity," a single platform to manage your security posture and streamline compliance frameworks like SOC 2 and HIPAA.
>
...
VarunaX is available in two editions:

| Edition | License | Best For |
|---------|---------|----------|
| **VarunaX Lite** | AGPL-3.0 | Security teams, independent researchers, testing your own applications |
| **VarunaX Pro** | Commercial | Enterprises requiring advanced features, CI/CD integration, and dedicated support |

> **This repository contains VarunaX Lite,** which utilizes our core autonomous AI pentesting framework. **VarunaX Pro** enhances this foundation with an advanced, LLM-powered data flow analysis engine (inspired by the [LLMDFA paper](https://arxiv.org/abs/2402.10754)) for enterprise-grade code analysis and deeper vulnerability detection.

> [!IMPORTANT]
> **White-box only.** VarunaX Lite is designed for **white-box (source-available)** application security testing.  
> It expects access to your application's source code and repository layout.

[See feature comparison](./VARUNAX-PRO.md)
## 📑 Table of Contents

- [What is VarunaX?](#-what-is-varunax)
- [See VarunaX in Action](#-see-varunax-in-action)
- [Features](#-features)
...
- [Setup & Usage Instructions](#-setup--usage-instructions)
  ...
  - [Stopping VarunaX](#stopping-varunax)
...
```bash
# 1. Clone VarunaX
git clone https://github.com/KeygraphHQ/varunax.git
cd varunax
...
# 3. Run a pentest
./varunax start URL=https://your-app.com REPO=your-repo
```

VarunaX will build the containers, start the workflow, and return a workflow ID. The pentest runs in the background.
...
./varunax query ID=varunax-1234567890
...
```bash
# Basic pentest
./varunax start URL=https://example.com REPO=repo-name

# With a configuration file
./varunax start URL=https://example.com REPO=repo-name CONFIG=./configs/my-config.yaml

# Custom output directory
./varunax start URL=https://example.com REPO=repo-name OUTPUT=./my-reports
```

### Prepare Your Repository

VarunaX expects target repositories to be placed under the `./repos/` directory at the project root. The `REPO` flag refers to a folder name inside `./repos/`. Copy the repository you want to scan into `./repos/`, or clone it directly there:
...
```bash
./varunax start URL=http://host.docker.internal:3000 REPO=repo-name
```
...
VarunaX can experimentally route requests through alternative AI providers using claude-code-router. This mode is not officially supported and is intended primarily for:

* **Model experimentation** — try VarunaX with GPT-5.2 or Gemini 3–family models
...
```bash
./varunax start URL=https://example.com REPO=repo-name ROUTER=true
```
...
See VarunaX's capabilities in action with penetration test results from industry-standard vulnerable applications:
...
📄 **[View Complete Report →](sample-reports/varunax-report-juice-shop.md)**
...
📄 **[View Complete Report →](sample-reports/varunax-report-capital-api.md)**
...
📄 **[View Complete Report →](sample-reports/varunax-report-crapi.md)**

---

*These results demonstrate VarunaX's ability to move beyond simple scanning, performing deep contextual exploitation with minimal false positives and actionable proof-of-concepts.*

---

## 🏗️ Architecture

VarunaX emulates a human penetration tester's methodology using a sophisticated multi-agent architecture. It combines white-box source code analysis with black-box dynamic exploitation across four distinct phases:
...
For detailed information about VarunaX's security testing coverage and development roadmap, see our [Coverage and Roadmap](./COVERAGE.md) documentation.

## ⚠️ Disclaimers

### Important Usage Guidelines & Disclaimers

Please review the following guidelines carefully before using VarunaX (Lite). As a user, you are responsible for your actions and assume all liability.
...
> [!WARNING]
> **⚠️ DO NOT run VarunaX on production environments.**
>
...
VarunaX is designed for legitimate security auditing purposes only.

> [!CAUTION]
> **You must have explicit, written authorization** from the owner of the target system before running VarunaX.
>
> Unauthorized scanning and exploitation of systems you do not own is illegal and can be prosecuted under laws such as the Computer Fraud and Abuse Act (CFAA). Keygraph is not responsible for any misuse of VarunaX.
...
- **Comprehensiveness**: The analysis in VarunaX Lite may not be exhaustive due to the inherent limitations of LLM context windows. For a more comprehensive, graph-based analysis of your entire codebase, **VarunaX Pro** leverages its advanced data flow analysis engine to ensure deeper and more thorough coverage.

#### **4. Scope of Analysis**

- **Targeted Vulnerabilities**: The current version of VarunaX Lite specifically targets the following classes of *exploitable* vulnerabilities:
  - Broken Authentication & Authorization
  - Injection
  - Cross-Site Scripting (XSS)
  - Server-Side Request Forgery (SSRF)
- **What VarunaX Lite Does Not Cover**: This list is not exhaustive of all potential security risks. VarunaX Lite's "proof-by-exploitation" model means it will not report on issues it cannot actively exploit, such as vulnerable third-party libraries or insecure configurations. These types of deep static-analysis findings are a core focus of the advanced analysis engine in **VarunaX Pro**.
...
Windows Defender may flag files in `xben-benchmark-results/` or `deliverables/` as malware. These are false positives caused by exploit code in the reports. Add an exclusion for the VarunaX directory in Windows Defender, or use Docker/WSL2.


## 📜 License

VarunaX Lite is released under the [GNU Affero General Public License v3.0 (AGPL-3.0)](LICENSE).

VarunaX is open source (AGPL v3). This license allows you to:
- Use it freely for all internal security testing.
- Modify the code privately for internal use without sharing your changes.

The AGPL's sharing requirements primarily apply to organizations offering VarunaX as a public or managed service (such as a SaaS platform). In those specific cases, any modifications made to the core software must be open-sourced.
...
- 🐛 **Report bugs** via [GitHub Issues](https://github.com/KeygraphHQ/varunax/issues)
- 💡 **Suggest features** in [Discussions](https://github.com/KeygraphHQ/varunax/discussions)
...
For a detailed breakdown of features, technical differences, and enterprise use cases, see our [complete comparison guide](./VARUNAX-PRO.md).
...
<p align="center">
  <a href="https://docs.google.com/forms/d/e/1FAIpQLSf-cPZcWjlfBJ3TCT8AaWpf8ztsw3FaHzJE4urr55KdlQs6cQ/viewform?usp=header" target="_blank">
    <img src="https://img.shields.io/badge/📋%20Express%20Interest%20in%20VarunaX%20Pro-4285F4?style=for-the-badge&logo=google&logoColor=white" alt="Express Interest">
  </a>
</p>

**Or contact us directly:**

📧 **Email**: [varunax@keygraph.io](mailto:varunax@keygraph.io)
