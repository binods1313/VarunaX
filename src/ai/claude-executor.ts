...
import { createVarunaXHelperServer } from '../../mcp-server/dist/index.js';
...
declare global {
  var VARUNAX_DISABLE_LOADER: boolean | undefined;
}
...
type McpServer = ReturnType<typeof createVarunaXHelperServer> | StdioMcpServer;
...
function buildMcpServers(
  ...
  const varunaxHelperServer = createVarunaXHelperServer(sourceDir);

  const mcpServers: Record<string, McpServer> = {
    'varunax-helper': varunaxHelperServer,
  };
  ...
  if (agentName) {
    ...
    if (playwrightMcpName) {
      ...
      const isDocker = process.env.VARUNAX_DOCKER === 'true';
...
export async function runClaudePrompt(
  ...
  const progress = createProgressManager(
    ...
    global.VARUNAX_DISABLE_LOADER ?? false
  );
...
async function processMessageStream(
  ...
  for await (const message of query({ prompt: fullPrompt, options })) {
    ...
    const now = Date.now();
    if (global.VARUNAX_DISABLE_LOADER && now - lastHeartbeat > HEARTBEAT_INTERVAL) {
