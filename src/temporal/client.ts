...
/**
 * Temporal client for starting VarunaX pentest pipeline workflows.
 *
 ...
 *   --workflow-id <id>    Custom workflow ID (default: varunax-<timestamp>)
...
async function startPipeline(): Promise<void> {
  ...
  try {
    ...
    const workflowId = customWorkflowId || `${hostname}_varunax-${Date.now()}`;
    ...
    if (!waitForCompletion) {
      ...
      console.log(chalk.white('  Logs:    ') + chalk.gray(`./varunax logs ID=${workflowId}`));
      console.log(chalk.white('  Query:   ') + chalk.gray(`./varunax query ID=${workflowId}`));
      console.log();
